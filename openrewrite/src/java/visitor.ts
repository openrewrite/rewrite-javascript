...
export class JavaVisitor<P> extends TreeVisitor<J, P> {
    ...
    public visitAnnotatedType(annotatedType: java.AnnotatedType, p: P): J | null {
        annotatedType = annotatedType.withPrefix(this.visitSpace(annotatedType.prefix, SpaceLocation.ANNOTATED_TYPE_PREFIX, p)!);
    ...
    public visitAnnotation(annotation: java.Annotation, p: P): J | null {
        annotation = annotation.withPrefix(this.visitSpace(annotation.prefix, SpaceLocation.ANNOTATION_PREFIX, p)!);
        ...
        annotation = annotation.padding.withArguments(this.visitContainer(annotation.padding.arguments, JContainerLocation.ANNOTATION_ARGUMENTS, p));
        return annotation;
    }

    public visitArrayAccess(arrayAccess: java.ArrayAccess, p: P): J | null {
        arrayAccess = arrayAccess.withPrefix(this.visitSpace(arrayAccess.prefix, SpaceLocation.ARRAY_ACCESS_PREFIX, p)!);
    ...
    public visitArrayType(arrayType: java.ArrayType, p: P): J | null {
        arrayType = arrayType.withPrefix(this.visitSpace(arrayType.prefix, SpaceLocation.ARRAY_TYPE_PREFIX, p)!);
        ...
        arrayType = arrayType.withDimension(this.visitLeftPadded(arrayType.dimension, JLeftPaddedLocation.ARRAY_TYPE_DIMENSION, p));
        return arrayType;
    }

    public visitAssert(assert: java.Assert, p: P): J | null {
        assert = assert.withPrefix(this.visitSpace(assert.prefix, SpaceLocation.ASSERT_PREFIX, p)!);
        ...
        assert = assert.withDetail(this.visitLeftPadded(assert.detail, JLeftPaddedLocation.ASSERT_DETAIL, p));
        return assert;
    }

    public visitAssignment(assignment: java.Assignment, p: P): J | null {
        assignment = assignment.withPrefix(this.visitSpace(assignment.prefix, SpaceLocation.ASSIGNMENT_PREFIX, p)!);
        ...
        assignment = assignment.padding.withAssignment(this.visitLeftPadded(assignment.padding.assignment, JLeftPaddedLocation.ASSIGNMENT, p)!);
        return assignment;
    }

    public visitAssignmentOperation(assignmentOperation: java.AssignmentOperation, p: P): J | null {
        assignmentOperation = assignmentOperation.withPrefix(this.visitSpace(assignmentOperation.prefix, SpaceLocation.ASSIGNMENT_OPERATION_PREFIX, p)!);
        ...
        assignmentOperation = assignmentOperation.padding.withOperator(this.visitLeftPadded(assignmentOperation.padding.operator, JLeftPaddedLocation.ASSIGNMENT_OPERATION_OPERATOR, p)!);
    ...
    public visitBinary(binary: java.Binary, p: P): J | null {
        binary = binary.withPrefix(this.visitSpace(binary.prefix, SpaceLocation.BINARY_PREFIX, p)!);
        ...
        binary = binary.padding.withOperator(this.visitLeftPadded(binary.padding.operator, JLeftPaddedLocation.BINARY_OPERATOR, p)!);
        binary = binary.withRight(this.visitAndCast(binary.right, p)!);
        return binary;
    }

    public visitBlock(block: java.Block, p: P): J | null {
        block = block.withPrefix(this.visitSpace(block.prefix, SpaceLocation.BLOCK_PREFIX, p)!);
        ...
        block = block.padding.withStatic(this.visitRightPadded(block.padding.static, JRightPaddedLocation.STATIC_INIT, p)!);
        block = block.padding.withStatements(ListUtils.map(block.padding.statements, el => this.visitRightPadded(el, JRightPaddedLocation.BLOCK_STATEMENT, p)));
        block = block.withEnd(this.visitSpace(block.end, SpaceLocation.BLOCK_END, p)!);
        return block;
    }

    public visitBreak(_break: java.Break, p: P): J | null {
        _break = _break.withPrefix(this.visitSpace(_break.prefix, SpaceLocation.BREAK_PREFIX, p)!);
    ...
    public visitCase(_case: java.Case, p: P): J | null {
        _case = _case.withPrefix(this.visitSpace(_case.prefix, SpaceLocation.CASE_PREFIX, p)!);
        ...
        _case = _case.padding.withExpressions(this.visitContainer(_case.padding.expressions, JContainerLocation.CASE_EXPRESSION, p)!);
        _case = _case.padding.withStatements(this.visitContainer(_case.padding.statements, JContainerLocation.CASE, p)!);
        _case = _case.padding.withBody(this.visitRightPadded(_case.padding.body, JRightPaddedLocation.CASE_BODY, p));
        return _case;
    }

    public visitClassDeclaration(classDeclaration: java.ClassDeclaration, p: P): J | null {
        classDeclaration = classDeclaration.withPrefix(this.visitSpace(classDeclaration.prefix, SpaceLocation.CLASS_DECLARATION_PREFIX, p)!);
        ...
        classDeclaration = classDeclaration.padding.withTypeParameters(this.visitContainer(classDeclaration.padding.typeParameters, JContainerLocation.TYPE_PARAMETERS, p));
        classDeclaration = classDeclaration.padding.withPrimaryConstructor(this.visitContainer(classDeclaration.padding.primaryConstructor, JContainerLocation.RECORD_STATE_VECTOR, p));
        classDeclaration = classDeclaration.padding.withExtends(this.visitLeftPadded(classDeclaration.padding.extends, JLeftPaddedLocation.EXTENDS, p));
        classDeclaration = classDeclaration.padding.withImplements(this.visitContainer(classDeclaration.padding.implements, JContainerLocation.IMPLEMENTS, p));
        classDeclaration = classDeclaration.padding.withPermits(this.visitContainer(classDeclaration.padding.permits, JContainerLocation.PERMITS, p));
    ...
    public visitClassDeclarationKind(classDeclarationKind: java.ClassDeclarationKind, p: P): J | null {
        classDeclarationKind = classDeclarationKind.withPrefix(this.visitSpace(classDeclarationKind.prefix, SpaceLocation.CLASS_KIND, p)!);
    ...
    public visitCompilationUnit(compilationUnit: java.CompilationUnit, p: P): J | null {
        compilationUnit = compilationUnit.withPrefix(this.visitSpace(compilationUnit.prefix, SpaceLocation.COMPILATION_UNIT_PREFIX, p)!);
        compilationUnit = compilationUnit.withMarkers(this.visitMarkers(compilationUnit.markers, p));
        compilationUnit = compilationUnit.padding.withPackageDeclaration(this.visitRightPadded(compilationUnit.padding.packageDeclaration, JRightPaddedLocation.PACKAGE, p));
        compilationUnit = compilationUnit.padding.withImports(ListUtils.map(compilationUnit.padding.imports, el => this.visitRightPadded(el, JRightPaddedLocation.IMPORT, p)));
        ...
        compilationUnit = compilationUnit.withEof(this.visitSpace(compilationUnit.eof, SpaceLocation.COMPILATION_UNIT_EOF, p)!);
        return compilationUnit;
    }

    public visitContinue(_continue: java.Continue, p: P): J | null {
        _continue = _continue.withPrefix(this.visitSpace(_continue.prefix, SpaceLocation.CONTINUE_PREFIX, p)!);
    ...
    public visitDoWhileLoop(doWhileLoop: java.DoWhileLoop, p: P): J | null {
        doWhileLoop = doWhileLoop.withPrefix(this.visitSpace(doWhileLoop.prefix, SpaceLocation.DO_WHILE_PREFIX, p)!);
        ...
        doWhileLoop = doWhileLoop.padding.withBody(this.visitRightPadded(doWhileLoop.padding.body, JRightPaddedLocation.WHILE_BODY, p)!);
        doWhileLoop = doWhileLoop.padding.withWhileCondition(this.visitLeftPadded(doWhileLoop.padding.whileCondition, JLeftPaddedLocation.WHILE_CONDITION, p)!);
        return doWhileLoop;
    }

    public visitEmpty(empty: java.Empty, p: P): J | null {
        empty = empty.withPrefix(this.visitSpace(empty.prefix, SpaceLocation.EMPTY_PREFIX, p)!);
    ...
    public visitEnumValue(enumValue: java.EnumValue, p: P): J | null {
        enumValue = enumValue.withPrefix(this.visitSpace(enumValue.prefix, SpaceLocation.ENUM_VALUE_PREFIX, p)!);
    ...
    public visitEnumValueSet(enumValueSet: java.EnumValueSet, p: P): J | null {
        enumValueSet = enumValueSet.withPrefix(this.visitSpace(enumValueSet.prefix, SpaceLocation.ENUM_VALUE_SET_PREFIX, p)!);
        ...
        enumValueSet = enumValueSet.padding.withEnums(ListUtils.map(enumValueSet.padding.enums, el => this.visitRightPadded(el, JRightPaddedLocation.ENUM_VALUE, p)));
        return enumValueSet;
    }

    public visitFieldAccess(fieldAccess: java.FieldAccess, p: P): J | null {
        fieldAccess = fieldAccess.withPrefix(this.visitSpace(fieldAccess.prefix, SpaceLocation.FIELD_ACCESS_PREFIX, p)!);
        ...
        fieldAccess = fieldAccess.padding.withName(this.visitLeftPadded(fieldAccess.padding.name, JLeftPaddedLocation.FIELD_ACCESS_NAME, p)!);
        return fieldAccess;
    }

    public visitForEachLoop(forEachLoop: java.ForEachLoop, p: P): J | null {
        forEachLoop = forEachLoop.withPrefix(this.visitSpace(forEachLoop.prefix, SpaceLocation.FOR_EACH_LOOP_PREFIX, p)!);
        ...
        forEachLoop = forEachLoop.padding.withBody(this.visitRightPadded(forEachLoop.padding.body, JRightPaddedLocation.FOR_BODY, p)!);
        return forEachLoop;
    }

    public visitForEachControl(forEachLoopControl: java.ForEachLoopControl, p: P): J | null {
        forEachLoopControl = forEachLoopControl.withPrefix(this.visitSpace(forEachLoopControl.prefix, SpaceLocation.FOR_EACH_CONTROL_PREFIX, p)!);
        forEachLoopControl = forEachLoopControl.withMarkers(this.visitMarkers(forEachLoopControl.markers, p));
        forEachLoopControl = forEachLoopControl.padding.withVariable(this.visitRightPadded(forEachLoopControl.padding.variable, JRightPaddedLocation.FOREACH_VARIABLE, p)!);
        forEachLoopControl = forEachLoopControl.padding.withIterable(this.visitRightPadded(forEachLoopControl.padding.iterable, JRightPaddedLocation.FOREACH_ITERABLE, p)!);
        return forEachLoopControl;
    }

    public visitForLoop(forLoop: java.ForLoop, p: P): J | null {
        forLoop = forLoop.withPrefix(this.visitSpace(forLoop.prefix, SpaceLocation.FOR_PREFIX, p)!);
        ...
        forLoop = forLoop.padding.withBody(this.visitRightPadded(forLoop.padding.body, JRightPaddedLocation.FOR_BODY, p)!);
        return forLoop;
    }

    public visitForControl(forLoopControl: java.ForLoopControl, p: P): J | null {
        forLoopControl = forLoopControl.withPrefix(this.visitSpace(forLoopControl.prefix, SpaceLocation.FOR_CONTROL_PREFIX, p)!);
        forLoopControl = forLoopControl.withMarkers(this.visitMarkers(forLoopControl.markers, p));
        forLoopControl = forLoopControl.padding.withInit(ListUtils.map(forLoopControl.padding.init, el => this.visitRightPadded(el, JRightPaddedLocation.FOR_INIT, p)));
        forLoopControl = forLoopControl.padding.withCondition(this.visitRightPadded(forLoopControl.padding.condition, JRightPaddedLocation.FOR_CONDITION, p)!);
        forLoopControl = forLoopControl.padding.withUpdate(ListUtils.map(forLoopControl.padding.update, el => this.visitRightPadded(el, JRightPaddedLocation.FOR_UPDATE, p)));
        return forLoopControl;
    }

    public visitParenthesizedTypeTree(parenthesizedTypeTree: java.ParenthesizedTypeTree, p: P): J | null {
        parenthesizedTypeTree = parenthesizedTypeTree.withPrefix(this.visitSpace(parenthesizedTypeTree.prefix, SpaceLocation.PARENTHESES_PREFIX, p)!);
    ...
    public visitIdentifier(identifier: java.Identifier, p: P): J | null {
        identifier = identifier.withPrefix(this.visitSpace(identifier.prefix, SpaceLocation.IDENTIFIER_PREFIX, p)!);
    ...
    public visitIf(_if: java.If, p: P): J | null {
        _if = _if.withPrefix(this.visitSpace(_if.prefix, SpaceLocation.IF_PREFIX, p)!);
        ...
        _if = _if.padding.withThenPart(this.visitRightPadded(_if.padding.thenPart, JRightPaddedLocation.IF_THEN, p)!);
        _if = _if.withElsePart(this.visitAndCast(_if.elsePart, p));
        return _if;
    }

    public visitElse(ifElse: java.IfElse, p: P): J | null {
        ifElse = ifElse.withPrefix(this.visitSpace(ifElse.prefix, SpaceLocation.ELSE_PREFIX, p)!);
        ifElse = ifElse.withMarkers(this.visitMarkers(ifElse.markers, p));
        ifElse = ifElse.padding.withBody(this.visitRightPadded(ifElse.padding.body, JRightPaddedLocation.IF_ELSE, p)!);
        return ifElse;
    }

    public visitImport(_import: java.Import, p: P): J | null {
        _import = _import.withPrefix(this.visitSpace(_import.prefix, SpaceLocation.IMPORT_PREFIX, p)!);
        ...
        _import = _import.padding.withStatic(this.visitLeftPadded(_import.padding.static, JLeftPaddedLocation.STATIC_IMPORT, p)!);
        _import = _import.withQualid(this.visitAndCast(_import.qualid, p)!);
        _import = _import.padding.withAlias(this.visitLeftPadded(_import.padding.alias, JLeftPaddedLocation.IMPORT_ALIAS_PREFIX, p));
        return _import;
    }

    public visitInstanceOf(instanceOf: java.InstanceOf, p: P): J | null {
        instanceOf = instanceOf.withPrefix(this.visitSpace(instanceOf.prefix, SpaceLocation.INSTANCEOF_PREFIX, p)!);
        ...
        instanceOf = instanceOf.padding.withExpression(this.visitRightPadded(instanceOf.padding.expression, JRightPaddedLocation.INSTANCEOF, p)!);
    ...
    public visitIntersectionType(intersectionType: java.IntersectionType, p: P): J | null {
        intersectionType = intersectionType.withPrefix(this.visitSpace(intersectionType.prefix, SpaceLocation.INTERSECTION_TYPE_PREFIX, p)!);
        ...
        intersectionType = intersectionType.padding.withBounds(this.visitContainer(intersectionType.padding.bounds, JContainerLocation.TYPE_BOUNDS, p)!);
        return intersectionType;
    }

    public visitLabel(label: java.Label, p: P): J | null {
        label = label.withPrefix(this.visitSpace(label.prefix, SpaceLocation.LABEL_PREFIX, p)!);
        ...
        label = label.padding.withLabel(this.visitRightPadded(label.padding.label, JRightPaddedLocation.LABEL, p)!);
        label = label.withStatement(this.visitAndCast(label.statement, p)!);
        return label;
    }

    public visitLambda(lambda: java.Lambda, p: P): J | null {
        lambda = lambda.withPrefix(this.visitSpace(lambda.prefix, SpaceLocation.LAMBDA_PREFIX, p)!);
        ...
        lambda = lambda.withArrow(this.visitSpace(lambda.arrow, SpaceLocation.LAMBDA_ARROW_PREFIX, p)!);
    ...
    public visitLambdaParameters(lambdaParameters: java.LambdaParameters, p: P): J | null {
        lambdaParameters = lambdaParameters.withPrefix(this.visitSpace(lambdaParameters.prefix, SpaceLocation.LAMBDA_PARAMETERS_PREFIX, p)!);
        lambdaParameters = lambdaParameters.withMarkers(this.visitMarkers(lambdaParameters.markers, p));
        lambdaParameters = lambdaParameters.padding.withParameters(ListUtils.map(lambdaParameters.padding.parameters, el => this.visitRightPadded(el, JRightPaddedLocation.LAMBDA_PARAM, p)));
        return lambdaParameters;
    }

    public visitLiteral(literal: java.Literal, p: P): J | null {
        literal = literal.withPrefix(this.visitSpace(literal.prefix, SpaceLocation.LITERAL_PREFIX, p)!);
    ...
    public visitMemberReference(memberReference: java.MemberReference, p: P): J | null {
        memberReference = memberReference.withPrefix(this.visitSpace(memberReference.prefix, SpaceLocation.MEMBER_REFERENCE_PREFIX, p)!);
        ...
        memberReference = memberReference.padding.withContaining(this.visitRightPadded(memberReference.padding.containing, JRightPaddedLocation.MEMBER_REFERENCE_CONTAINING, p)!);
        memberReference = memberReference.padding.withTypeParameters(this.visitContainer(memberReference.padding.typeParameters, JContainerLocation.TYPE_PARAMETERS, p));
        memberReference = memberReference.padding.withReference(this.visitLeftPadded(memberReference.padding.reference, JLeftPaddedLocation.MEMBER_REFERENCE_NAME, p)!);
        return memberReference;
    }

    public visitMethodDeclaration(methodDeclaration: java.MethodDeclaration, p: P): J | null {
        methodDeclaration = methodDeclaration.withPrefix(this.visitSpace(methodDeclaration.prefix, SpaceLocation.METHOD_DECLARATION_PREFIX, p)!);
        ...
        methodDeclaration = methodDeclaration.padding.withParameters(this.visitContainer(methodDeclaration.padding.parameters, JContainerLocation.METHOD_DECLARATION_PARAMETERS, p)!);
        methodDeclaration = methodDeclaration.padding.withThrows(this.visitContainer(methodDeclaration.padding.throws, JContainerLocation.THROWS, p));
        methodDeclaration = methodDeclaration.withBody(this.visitAndCast(methodDeclaration.body, p));
        methodDeclaration = methodDeclaration.padding.withDefaultValue(this.visitLeftPadded(methodDeclaration.padding.defaultValue, JLeftPaddedLocation.METHOD_DECLARATION_DEFAULT_VALUE, p));
        return methodDeclaration;
    }

    public visitMethodInvocation(methodInvocation: java.MethodInvocation, p: P): J | null {
        methodInvocation = methodInvocation.withPrefix(this.visitSpace(methodInvocation.prefix, SpaceLocation.METHOD_INVOCATION_PREFIX, p)!);
        ...
        methodInvocation = methodInvocation.padding.withSelect(this.visitRightPadded(methodInvocation.padding.select, JRightPaddedLocation.METHOD_SELECT, p));
        methodInvocation = methodInvocation.padding.withTypeParameters(this.visitContainer(methodInvocation.padding.typeParameters, JContainerLocation.TYPE_PARAMETERS, p));
        methodInvocation = methodInvocation.withName(this.visitAndCast(methodInvocation.name, p)!);
        methodInvocation = methodInvocation.padding.withArguments(this.visitContainer(methodInvocation.padding.arguments, JContainerLocation.METHOD_INVOCATION_ARGUMENTS, p)!);
        return methodInvocation;
    }

    public visitModifier(modifier: java.Modifier, p: P): J | null {
        modifier = modifier.withPrefix(this.visitSpace(modifier.prefix, SpaceLocation.MODIFIER_PREFIX, p)!);
    ...
    public visitMultiCatch(multiCatch: java.MultiCatch, p: P): J | null {
        multiCatch = multiCatch.withPrefix(this.visitSpace(multiCatch.prefix, SpaceLocation.MULTI_CATCH_PREFIX, p)!);
        multiCatch = multiCatch.withMarkers(this.visitMarkers(multiCatch.markers, p));
        multiCatch = multiCatch.padding.withAlternatives(ListUtils.map(multiCatch.padding.alternatives, el => this.visitRightPadded(el, JRightPaddedLocation.CATCH_ALTERNATIVE, p)));
        return multiCatch;
    }

    public visitNewArray(newArray: java.NewArray, p: P): J | null {
        newArray = newArray.withPrefix(this.visitSpace(newArray.prefix, SpaceLocation.NEW_ARRAY_PREFIX, p)!);
        ...
        newArray = newArray.padding.withInitializer(this.visitContainer(newArray.padding.initializer, JContainerLocation.NEW_ARRAY_INITIALIZER, p));
        return newArray;
    }

    public visitArrayDimension(arrayDimension: java.ArrayDimension, p: P): J | null {
        arrayDimension = arrayDimension.withPrefix(this.visitSpace(arrayDimension.prefix, SpaceLocation.DIMENSION_PREFIX, p)!);
        arrayDimension = arrayDimension.withMarkers(this.visitMarkers(arrayDimension.markers, p));
        arrayDimension = arrayDimension.padding.withIndex(this.visitRightPadded(arrayDimension.padding.index, JRightPaddedLocation.ARRAY_INDEX, p)!);
        return arrayDimension;
    }

    public visitNewClass(newClass: java.NewClass, p: P): J | null {
        newClass = newClass.withPrefix(this.visitSpace(newClass.prefix, SpaceLocation.NEW_CLASS_PREFIX, p)!);
        ...
        newClass = newClass.padding.withEnclosing(this.visitRightPadded(newClass.padding.enclosing, JRightPaddedLocation.NEW_CLASS_ENCLOSING, p));
        newClass = newClass.withNew(this.visitSpace(newClass.new, SpaceLocation.NEW_PREFIX, p)!);
        newClass = newClass.withClazz(this.visitAndCast(newClass.clazz, p));
        newClass = newClass.padding.withArguments(this.visitContainer(newClass.padding.arguments, JContainerLocation.NEW_CLASS_ARGUMENTS, p)!);
    ...
    public visitNullableType(nullableType: java.NullableType, p: P): J | null {
        nullableType = nullableType.withPrefix(this.visitSpace(nullableType.prefix, SpaceLocation.NULLABLE_TYPE_PREFIX, p)!);
        ...
        nullableType = nullableType.padding.withTypeTree(this.visitRightPadded(nullableType.padding.typeTree, JRightPaddedLocation.NULLABLE, p)!);
        return nullableType;
    }

    public visitPackage(_package: java.Package, p: P): J | null {
        _package = _package.withPrefix(this.visitSpace(_package.prefix, SpaceLocation.PACKAGE_PREFIX, p)!);
    ...
    public visitParameterizedType(parameterizedType: java.ParameterizedType, p: P): J | null {
        parameterizedType = parameterizedType.withPrefix(this.visitSpace(parameterizedType.prefix, SpaceLocation.PARAMETERIZED_TYPE_PREFIX, p)!);
        ...
        parameterizedType = parameterizedType.padding.withTypeParameters(this.visitContainer(parameterizedType.padding.typeParameters, JContainerLocation.TYPE_PARAMETERS, p));
        return parameterizedType;
    }

    public visitParentheses<J2 extends J>(parentheses: java.Parentheses<J2>, p: P): J | null {
        parentheses = parentheses.withPrefix(this.visitSpace(parentheses.prefix, SpaceLocation.PARENTHESES_PREFIX, p)!);
        ...
        parentheses = parentheses.padding.withTree(this.visitRightPadded(parentheses.padding.tree, JRightPaddedLocation.PARENTHESES, p)!);
        return parentheses;
    }

    public visitControlParentheses<J2 extends J>(controlParentheses: java.ControlParentheses<J2>, p: P): J | null {
        controlParentheses = controlParentheses.withPrefix(this.visitSpace(controlParentheses.prefix, SpaceLocation.CONTROL_PARENTHESES_PREFIX, p)!);
        ...
        controlParentheses = controlParentheses.padding.withTree(this.visitRightPadded(controlParentheses.padding.tree, JRightPaddedLocation.PARENTHESES, p)!);
        return controlParentheses;
    }

    public visitPrimitive(primitive: java.Primitive, p: P): J | null {
        primitive = primitive.withPrefix(this.visitSpace(primitive.prefix, SpaceLocation.PRIMITIVE_PREFIX, p)!);
    ...
    public visitReturn(_return: java.Return, p: P): J | null {
        _return = _return.withPrefix(this.visitSpace(_return.prefix, SpaceLocation.RETURN_PREFIX, p)!);
    ...
    public visitSwitch(_switch: java.Switch, p: P): J | null {
        _switch = _switch.withPrefix(this.visitSpace(_switch.prefix, SpaceLocation.SWITCH_PREFIX, p)!);
    ...
    public visitSwitchExpression(switchExpression: java.SwitchExpression, p: P): J | null {
        switchExpression = switchExpression.withPrefix(this.visitSpace(switchExpression.prefix, SpaceLocation.SWITCH_EXPRESSION_PREFIX, p)!);
    ...
    public visitSynchronized(synchronized: java.Synchronized, p: P): J | null {
        synchronized = synchronized.withPrefix(this.visitSpace(synchronized.prefix, SpaceLocation.SYNCHRONIZED_PREFIX, p)!);
    ...
    public visitTernary(ternary: java.Ternary, p: P): J | null {
        ternary = ternary.withPrefix(this.visitSpace(ternary.prefix, SpaceLocation.TERNARY_PREFIX, p)!);
        ...
        ternary = ternary.padding.withTruePart(this.visitLeftPadded(ternary.padding.truePart, JLeftPaddedLocation.TERNARY_TRUE, p)!);
        ternary = ternary.padding.withFalsePart(this.visitLeftPadded(ternary.padding.falsePart, JLeftPaddedLocation.TERNARY_FALSE, p)!);
        return ternary;
    }

    public visitThrow(_throw: java.Throw, p: P): J | null {
        _throw = _throw.withPrefix(this.visitSpace(_throw.prefix, SpaceLocation.THROW_PREFIX, p)!);
    ...
    public visitTry(_try: java.Try, p: P): J | null {
        _try = _try.withPrefix(this.visitSpace(_try.prefix, SpaceLocation.TRY_PREFIX, p)!);
        ...
        _try = _try.padding.withResources(this.visitContainer(_try.padding.resources, JContainerLocation.TRY_RESOURCES, p));
        ...
        _try = _try.padding.withFinally(this.visitLeftPadded(_try.padding.finally, JLeftPaddedLocation.TRY_FINALLY, p));
        return _try;
    }

    public visitTryResource(tryResource: java.TryResource, p: P): J | null {
        tryResource = tryResource.withPrefix(this.visitSpace(tryResource.prefix, SpaceLocation.TRY_RESOURCE, p)!);
    ...
    public visitCatch(tryCatch: java.TryCatch, p: P): J | null {
        tryCatch = tryCatch.withPrefix(this.visitSpace(tryCatch.prefix, SpaceLocation.CATCH_PREFIX, p)!);
    ...
    public visitTypeCast(typeCast: java.TypeCast, p: P): J | null {
        typeCast = typeCast.withPrefix(this.visitSpace(typeCast.prefix, SpaceLocation.TYPE_CAST_PREFIX, p)!);
    ...
    public visitTypeParameter(typeParameter: java.TypeParameter, p: P): J | null {
        typeParameter = typeParameter.withPrefix(this.visitSpace(typeParameter.prefix, SpaceLocation.TYPE_PARAMETERS_PREFIX, p)!);
        ...
        typeParameter = typeParameter.padding.withBounds(this.visitContainer(typeParameter.padding.bounds, JContainerLocation.TYPE_BOUNDS, p));
        return typeParameter;
    }

    public visitTypeParameters(typeParameters: java.TypeParameters, p: P): J | null {
        typeParameters = typeParameters.withPrefix(this.visitSpace(typeParameters.prefix, SpaceLocation.TYPE_PARAMETERS_PREFIX, p)!);
        ...
        typeParameters = typeParameters.padding.withTypeParameters(ListUtils.map(typeParameters.padding.typeParameters, el => this.visitRightPadded(el, JRightPaddedLocation.TYPE_PARAMETER, p)));
        return typeParameters;
    }

    public visitUnary(unary: java.Unary, p: P): J | null {
        unary = unary.withPrefix(this.visitSpace(unary.prefix, SpaceLocation.UNARY_PREFIX, p)!);
        ...
        unary = unary.padding.withOperator(this.visitLeftPadded(unary.padding.operator, JLeftPaddedLocation.UNARY_OPERATOR, p)!);
    ...
    public visitVariableDeclarations(variableDeclarations: java.VariableDeclarations, p: P): J | null {
        variableDeclarations = variableDeclarations.withPrefix(this.visitSpace(variableDeclarations.prefix, SpaceLocation.VARIABLE_DECLARATIONS_PREFIX, p)!);
        ...
        variableDeclarations = variableDeclarations.withVarargs(this.visitSpace(variableDeclarations.varargs, SpaceLocation.VARARGS, p));
        ...
        variableDeclarations = variableDeclarations.padding.withVariables(ListUtils.map(variableDeclarations.padding.variables, el => this.visitRightPadded(el, JRightPaddedLocation.NAMED_VARIABLE, p)));
        return variableDeclarations;
    }

    public visitVariable(variableDeclarationsNamedVariable: java.VariableDeclarationsNamedVariable, p: P): J | null {
        variableDeclarationsNamedVariable = variableDeclarationsNamedVariable.withPrefix(this.visitSpace(variableDeclarationsNamedVariable.prefix, SpaceLocation.VARIABLE_PREFIX, p)!);
        ...
        variableDeclarationsNamedVariable = variableDeclarationsNamedVariable.padding.withInitializer(this.visitLeftPadded(variableDeclarationsNamedVariable.padding.initializer, JLeftPaddedLocation.VARIABLE_INITIALIZER, p));
        return variableDeclarationsNamedVariable;
    }

    public visitWhileLoop(whileLoop: java.WhileLoop, p: P): J | null {
        whileLoop = whileLoop.withPrefix(this.visitSpace(whileLoop.prefix, SpaceLocation.WHILE_PREFIX, p)!);
        ...
        whileLoop = whileLoop.padding.withBody(this.visitRightPadded(whileLoop.padding.body, JRightPaddedLocation.WHILE_BODY, p)!);
        return whileLoop;
    }

    public visitWildcard(wildcard: java.Wildcard, p: P): J | null {
        wildcard = wildcard.withPrefix(this.visitSpace(wildcard.prefix, SpaceLocation.WILDCARD_PREFIX, p)!);
        ...
        wildcard = wildcard.padding.withBound(this.visitLeftPadded(wildcard.padding.bound, JLeftPaddedLocation.WILDCARD_BOUND, p));
        wildcard = wildcard.withBoundedType(this.visitAndCast(wildcard.boundedType, p));
        return wildcard;
    }

    public visitYield(_yield: java.Yield, p: P): J | null {
        _yield = _yield.withPrefix(this.visitSpace(_yield.prefix, SpaceLocation.YIELD_PREFIX, p)!);
    ...
    public visitUnknown(unknown: java.Unknown, p: P): J | null {
        unknown = unknown.withPrefix(this.visitSpace(unknown.prefix, SpaceLocation.UNKNOWN_PREFIX, p)!);
    ...
    public visitUnknownSource(unknownSource: java.UnknownSource, p: P): J | null {
        unknownSource = unknownSource.withPrefix(this.visitSpace(unknownSource.prefix, SpaceLocation.UNKNOWN_SOURCE_PREFIX, p)!);
