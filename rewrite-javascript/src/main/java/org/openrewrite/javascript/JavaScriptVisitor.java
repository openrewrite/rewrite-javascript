...
public class JavaScriptVisitor<P> extends JavaVisitor<P> {
    ...
    public J visitFunctionType(JS.FunctionType functionType, P p) {
        ...
        f = f.getPadding().withConstructorType(Objects.requireNonNull(visitRightPadded(f.getPadding().getConstructorType(), JsRightPadded.Location.FUNCTION_TYPE_CONSTRUCTOR, p)));
        f = f.withTypeParameters(Objects.requireNonNull(visitAndCast(f.getTypeParameters(), p)));
    ...
    public J visitSatisfiesExpression(JS.SatisfiesExpression satisfiesExpression, P p) {
        ...
        sa = sa.withExpression(visitAndCast(sa.getExpression(), p));
        sa = sa.getPadding().withSatisfiesType(visitLeftPadded(sa.getPadding().getSatisfiesType(), JsLeftPadded.Location.SATISFIES_EXPRESSION_TYPE, p));
        return sa;
    }
    ...
    public J visitNamespaceDeclaration(JS.NamespaceDeclaration namespaceDeclaration, P p) {
        ...
        ns = ns.withBody(Objects.requireNonNull(visitAndCast(ns.getBody(), p)));
        return ns;
    }
